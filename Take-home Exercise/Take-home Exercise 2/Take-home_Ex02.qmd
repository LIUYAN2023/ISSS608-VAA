...
1.  In the section "3.1 Interactive Network Graph with visNetwork," we focused on analyzing the network using a single HScode. As a next step, we can enhance the analysis by developing a Shiny app that offers a more flexible dropdown list. This will allow users to select different HScodes and Years to explore and understand the changing network patterns.

2.  In the section "3.2 Ego-Network for Key Node," we visualized the ego-network for a specific company. However, to provide a more comprehensive analysis, we can further develop the Shiny app to include a dropdown list. This dropdown list will enable users to select different companies and visualize their respective ego-networks, providing insights into the network structures around each company.

3.  This analysis primarily focused on Task 1. To further leverage the predicted knowledge graph links provided by FishEye, we can identify new patterns and anomalies. Additionally, we can use these patterns to detect companies that exhibit behaviors indicative of illegal fishing activities.

# 5. Reference

1.  Andrew, Stefano, 'Social Network Analysis of Global Transshipment:A Framework for Discovering Illegal Fishing', ASONAM, 2020([link](https://web.ntpu.edu.tw/~myday/doc/ASONAM2020/ASONAM2020_Proceedings/pdf/papers/122_143_776.pdf))
2.  Kittiya Kositduriyapun,'Social Network Analysis with R : Centrality Measure',2019([link](https://medium.com/@615162020004/social-network-analysis-with-r-centrality-measure-86d7fa273574))
3.  VisNetwork, an R package for interactive network visualization ([link](http://datastorm-open.github.io/visNetwork/))
4.  Harmonized System (HS) Codes ([link](https://www.trade.gov/harmonized-system-hs-codes))
